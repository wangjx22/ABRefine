...
def distmap_loss_function(pred_distmap, gt_model, start_model_backbone_mask, distmap_loss_fn, 
                          cdr1_mask, cdr2_mask, cdr3_mask, use_cdr_mask=False, sample_triplets=2000):
    # 提取骨干原子坐标
    ...
    if use_cdr_mask:
        for cdr_mask in [cdr1_mask, cdr2_mask, cdr3_mask]:
            cdr_mask = cdr_mask.to(gt_distmap.device)
            # 分类损失：筛选 CDR 区域内的残差对
            mask = cdr_mask[rows] & cdr_mask[cols]
            if mask.any():
                class_loss += distmap_loss_fn(pred_distmap[mask], final_bins[mask])
            
            # 三角损失：仅采样 CDR 区域内的节点
            nodes = torch.where(cdr_mask)[0]
            if len(nodes) >= 3:
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            tran_loss, angle_loss, cg_loss_all, tran_mse_loss = loss_function_T_backbone(
                ...
                align_atom_positions= data.align_atom_positions.to(local_rank),
            ...
            loss = weight_tran * tran_loss + weight_angle * angle_loss + cg_loss_weight * cg_loss_all + distmap_loss_weight *distmap_loss + tri_loss + tran_mse_loss
