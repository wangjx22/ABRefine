...
experiment_name=run_train_sin_k8s_FM_trial1
yaml_path=${PROJECT_DIR}/yamls/refine_train_yamls/run_train_sin_k8s_FM_trial1.yaml
...
max_lr=0.0001
output_dir=/nfs_beijing_ai/jinxian/rama-scoring1.3.0/run_train_sin_k8s_FM_trial1/
# mkdir -p ${output_dir}
#launch_mode=$1
...
if [[ x$launch_mode == xlocal ]]; then
    CUDA_VISIBLE_DEVICES=${device_ids} deepspeed --include="localhost:${device_ids}" --master_port=12342 ${PROJECT_DIR}/run_train_sin_FM.py \
        --deepspeed-init-dist \
    ...
    torchrun --nproc_per_node=${gpu_count} ${PROJECT_DIR}/run_train_sin_FM.py \
        --deepspeed-init-dist \
        --deepspeed_config ${PROJECT_DIR}/deepspeed_config.json \
        --yaml-path $yaml_path \
        --epochs $epochs \
