...
experiment_name=run_train_sin_k8s_trial5
...
max_lr=0.0001
output_dir=/nfs_beijing_ai/jinxian/rama-scoring1.3.0/trial5/
# mkdir -p ${output_dir}
#launch_mode=$1
...
if [[ x$launch_mode == xlocal ]]; then
    ...
    torchrun --nproc_per_node=${gpu_count} ${PROJECT_DIR}/run_train_sin_check_trans_output.py \
        --deepspeed-init-dist \
        --deepspeed_config ${PROJECT_DIR}/deepspeed_config.json \
        --yaml-path $yaml_path \
        --epochs $epochs \
