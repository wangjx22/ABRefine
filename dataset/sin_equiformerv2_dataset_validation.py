...
def triangle_inequality_loss(expected_dist, N, sample_triplets=500):
    ...
    # 生成随机三元组
    while True:
        i_indices = torch.randint(0, N, (sample_triplets,), device=device)
        j_indices = torch.randint(0, N, (sample_triplets,), device=device)
        k_indices = torch.randint(0, N, (sample_triplets,), device=device)
        
        # 确保三个点各不相同
        mask = (i_indices != j_indices) & (j_indices != k_indices) & (i_indices != k_indices)
        if mask.sum() >= sample_triplets // 2:
            i_indices = i_indices[mask][:sample_triplets//2]
            j_indices = j_indices[mask][:sample_triplets//2]
            k_indices = k_indices[mask][:sample_triplets//2]
            break
...
def triangle_inequality_loss_for_cdr(expected_dist, N, cdr_mask):
    """计算CDR区域的三角不等式损失（完整版本）"""
    device = expected_dist.device
    
    # 获取CDR区域的索引
    cdr_indices = torch.where(cdr_mask)[0]
    if len(cdr_indices) < 3:  # 至少需要3个点才能形成三角形
    ...
    for i in range(len(cdr_indices)-2):
        for j in range(i+1, len(cdr_indices)-1):
            for k in range(j+1, len(cdr_indices)):
                i_indices.append(cdr_indices[i])
                j_indices.append(cdr_indices[j])
                k_indices.append(cdr_indices[k])
    
    i_indices = torch.tensor(i_indices, device=device)
    j_indices = torch.tensor(j_indices, device=device)
    k_indices = torch.tensor(k_indices, device=device)
...
def calculate_cdr_loss(logits, true_distances, triu_indices, cdr_mask, N, distmap_loss_fn, bin_edges):
    """计算CDR区域的损失"""
    device = logits.device
    
    # 创建pair mask
    pair_mask = (cdr_mask[triu_indices[0]] | cdr_mask[triu_indices[1]])
    
    if not pair_mask.any():
        return torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)
    
    # 获取CDR区域对应的logits和distances
    cdr_logits = logits[pair_mask]
    cdr_distances = true_distances[pair_mask]
    
    # 计算bin indices
    bin_indices = torch.bucketize(cdr_distances, bin_edges.to(device))
    is_le_2 = (cdr_distances <= MIN_DIST)
    is_gt_20 = (cdr_distances > MAX_DIST)
    final_bins = bin_indices + 1
    final_bins[is_le_2] = 0
    final_bins[is_gt_20] = len(bin_edges) + 1
    
    # 计算losses
    class_loss = distmap_loss_fn(cdr_logits, final_bins)
    expected_dist = get_expected_distance(logits, bin_edges)
    tri_loss = triangle_inequality_loss_for_cdr(expected_dist, N, cdr_mask)
    
    return class_loss, tri_loss

def distmap_loss_function(pred_distmap, gt_model, start_model_backbone_mask, distmap_loss_fn, 
                         cdr1_mask, cdr2_mask, cdr3_mask, use_cdr_mask=False):
    """主损失函数"""
    device = pred_distmap.device
    ...
    true_distances = gt_distmap[triu_indices[0], triu_indices[1]]
    
    if not use_cdr_mask:
        # 计算整体loss
        bin_indices = torch.bucketize(true_distances, bin_edges)
        is_le_2 = (true_distances <= MIN_DIST)
        is_gt_20 = (true_distances > MAX_DIST)
        final_bins = bin_indices + 1
        final_bins[is_le_2] = 0
        final_bins[is_gt_20] = len(bin_edges) + 1
        ...
        tri_loss = triangle_inequality_loss(expected_dist, N, sample_triplets=2000)
        ...
        for mask in [cdr1_mask, cdr2_mask, cdr3_mask]:
            if mask is not None:
                class_loss, tri_loss = calculate_cdr_loss(
                    pred_distmap, true_distances, triu_indices, 
                    mask, N, distmap_loss_fn, bin_edges
                )
                losses.append((class_loss, tri_loss))
        
        if not losses:
            return torch.tensor(0.0, device=device), torch.tensor(0.0, device=device)
        
        # 计算平均loss
        total_class_loss = sum(l[0] for l in losses) / len(losses)
        total_tri_loss = sum(l[1] for l in losses) / len(losses)
        
        return total_class_loss, total_tri_loss
...
#     for i in range(len(cdr_indices)):
#         for j in range(i+1, len(cdr_indices)):
#             for k in range(j+1, len(cdr_indices)):
#                 i_indices.append(cdr_indices[i])
#                 j_indices.append(cdr_indices[j])
#                 k_indices.append(cdr_indices[k])
    
#     if not i_indices:  # 如果没有有效的三角形组合
#         return torch.tensor(0.0, device=device)
...
# def calculate_cdr_loss(logits, true_distances, triu_indices, cdr_mask, N, distmap_loss_fn):
#     """
#     计算特定CDR区域的loss
#     """
#     device = logits.device
...
#     # 计算bin indices (使用向量化操作)
#     bin_indices = torch.bucketize(cdr_distances, bin_edges.to(device))
#     is_le_2 = (cdr_distances <= 2.0)
#     is_gt_20 = (cdr_distances > 20.0)
#     final_bins = bin_indices + 1
#     final_bins[is_le_2] = 0
#     final_bins[is_gt_20] = len(bin_edges) + 1
...
#     # 计算expected distance和triangle loss
#     expected_dist = get_expected_distance(logits, bin_edges)
#     tri_loss = triangle_inequality_loss_for_cdr(expected_dist, N, cdr_mask)
...
#     if not use_cdr_mask:
#         # 计算整体loss
#         bin_indices = torch.bucketize(true_distances, bin_edges.to(device))
...
#     else:
#         # 分别计算CDR1-3的loss
#         cdr1_class_loss, cdr1_tri_loss = calculate_cdr_loss(
#             pred_distmap, true_distances, triu_indices, cdr1_mask, N, distmap_loss_fn)
        
#         cdr2_class_loss, cdr2_tri_loss = calculate_cdr_loss(
#             pred_distmap, true_distances, triu_indices, cdr2_mask, N, distmap_loss_fn)
        
#         cdr3_class_loss, cdr3_tri_loss = calculate_cdr_loss(
#             pred_distmap, true_distances, triu_indices, cdr3_mask, N, distmap_loss_fn)
        
#         # 组合loss (可以根据需要调整权重)
#         total_class_loss = (cdr1_class_loss + cdr2_class_loss + cdr3_class_loss) / 3
#         total_tri_loss = (cdr1_tri_loss + cdr2_tri_loss + cdr3_tri_loss) / 3
        
#         return total_class_loss, total_tri_loss

# def distmap_loss_function(pred_distmap, gt_model, start_model_backbone_mask, distmap_loss_fn, cdr1_mask, cdr2_mask, cdr3_mask, use_cdr_mask=False):
#     """
...
def loss_function_T_backbone(align_tran, predicted_tran, res_names_list, gt_res_mask, start_model_backbone_mask, init_model, align_atom_positions, cdr_mask_by_res_mask=None, cdr1_mask=None, cdr2_mask=None, cdr3_mask=None, config_runtime=None):
    """
    ...
    if config_runtime['use_cg_loss']:
        ...
        # 使用cdr_mask = cdr1_mask + cdr2_mask + cdr3_mask
        # cdr_mask = cdr1_mask + cdr2_mask + cdr3_mask
        # cdr_mask = cdr_mask.view(-1, 4)
        # cdr_mask = (torch.sum(cdr_mask, dim=1) == 4).long()

        # print('refine_backbone_atom:', refine_backbone_atom.shape)
        # print('gt_res_mask:', gt_res_mask.shape)
        # print('cdr_mask_by_res_mask:', cdr_mask_by_res_mask.shape)
