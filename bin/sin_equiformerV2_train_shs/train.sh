...
export WANDB_DIR=/nfs_beijing_ai/jinxian/rama-scoring1.3.0/logs
export WANDB_IGNORE_GIT=1

wandb_key=b00f1fee3fce213cd72cfeb42122329d758bf176
wandb_name=NB_EquiformerV2_train
wandb login --relogin $wandb_key

PROJECT_DIR=$(cd "$(dirname $0)" && pwd)/../../

trial=59 # which trial to train
experiment_name=run_train_sin_k8s_trial${trial}
yaml_path=${PROJECT_DIR}/yamls/refine_train_yamls/run_train_sin_k8s_trial${trial}.yaml
device_ids=0         # not used for k8s running.
gpus=1
epochs=100       # not used, controlled by max_epochs
max_epochs=100
log_interval=200
eval_interval=2000
...
max_lr=0.0001
output_dir=/nfs_beijing_ai/jinxian/rama-scoring1.3.0/trial${trial}/
# mkdir -p ${output_dir}
#launch_mode=$1
...
if [[ x$launch_mode == xlocal ]]; then
    CUDA_VISIBLE_DEVICES=${device_ids} deepspeed --include="localhost:${device_ids}" --master_port=12342 ${PROJECT_DIR}/run_train_sin_split_tran_and_angle.py \
        --deepspeed-init-dist \
    ...
    torchrun --nproc_per_node=${gpu_count} ${PROJECT_DIR}/run_train_sin_split_tran_and_angle.py \
        --deepspeed-init-dist \
        --deepspeed_config ${PROJECT_DIR}/deepspeed_config.json \
        --yaml-path $yaml_path \
        --epochs $epochs \
