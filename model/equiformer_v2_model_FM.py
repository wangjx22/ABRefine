    ...
    def forward(self, data, t):
        ...
        for i in range(self.num_resolutions):
            if self.num_resolutions == 1:
                # add t embedding in the offset_res==0
                x.embedding[:, offset_res, :] = torch.cat([self.element_sphere_embedding(atomic_numbers),
                                                           ...
                                                           self.resid_sphere_embedding(resid)], dim=-1) + (self.timestep_embedding(timesteps=t, dim=self.sphere_channels_all).squeeze(1).repeat(1, 3))
        ...
        if self.use_invariant_block:
            ...
            return pred_distmap, atom_embedding
        else:
            return None, atom_embedding
